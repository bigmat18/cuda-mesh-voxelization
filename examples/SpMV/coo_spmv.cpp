/* SpMV implementation with COO sparse matrices */
...
int main(int argc, char **argv)
{
    ...
    coo_matrix host_A;
    gen_sparseMatrix_COO(&host_A, L, density);
    ...
    gpuErrchk(cudaMalloc((void**) &(tmp_A.rowIdx), host_A.non_zeros*sizeof(int)));
    ...
    gpuErrchk(cudaMemcpy(tmp_A.rowIdx, host_A.rowIdx, host_A.non_zeros*sizeof(int), cudaMemcpyHostToDevice));
    ...
    coo_matrix *dev_A;
    gpuErrchk(cudaMalloc((void**) &dev_A, sizeof(coo_matrix)));
    gpuErrchk(cudaMemcpy(dev_A, &tmp_A, sizeof(coo_matrix), cudaMemcpyHostToDevice));
    ...
    unsigned int numBlocks = std::ceil(((double) host_A.non_zeros) / B);
    coo_spmv<<<numBlocks, B>>>(dev_A, dev_b, dev_c);
    ...
    gpuErrchk(cudaFree(tmp_A.rowIdx));
    ...
    for (int i=0; i<host_A.non_zeros; i++) {
        unsigned int row = host_A.rowIdx[i];
        unsigned int col = host_A.columnIdx[i];
        float value = host_A.value[i];
        check_c[row] += host_b[col]*value;
    }
