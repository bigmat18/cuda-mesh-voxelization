/* Solution of parallel convolution on GPU with tiling */
        ...
        if (tileX >= 0 && tileX < OUT_TILE_DIM && tileY >=0 && tileY < OUT_TILE_DIM) {
            ...
            for (int fy=0; fy<2*RADIUS+1; fy++) {
                for(int fx=0; fx<2*RADIUS+1; fx++) {
                    pvalue += F[fy * (2*RADIUS + 1) + fx] * A_smem[(tileY + fy)][(tileX + fx)];
                }
            }
            B[iy * w + ix] = pvalue;
        }
...
int main(int argc, char **argv)
{
    ...
    tiled_conv<<<gridDim, blockDim>>>(dev_A, dev_B, dev_F, L, L);
