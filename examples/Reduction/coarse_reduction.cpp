/* Solution of parallel reduction on GPU with shared memory and thread coarsening */

#include<iostream>
#include<stdlib.h>

#define COARSE_FACTOR 2 // coarsening factor (at least 1)
...
int main(int argc, char **argv)
{
    ...
    *host_result = 0;
    ...
    coarse_reduction<<<numBlocks, block_size, block_size * sizeof(int)>>>(dev_A, L, dev_result);
    ...
    if (check != *host_result) {
        ...
        std::cout << "Result gpu: " << *host_result << ", check: " << check << std::endl;
    }
    else {
